...
import { u as usePlatformPricing } from './usePlatformPricing-DwP11hj2.mjs';
import { _ as _export_sfc } from './server.mjs';
import { u as useStripe } from './useStripe-CrT1Y72L.mjs';
...
const useFinancialHealth = () => {
  ...
  const calculateBusinessHealth = async () => {
    ...
    const totalCash = (accounts == null ? void 0 : accounts.reduce((sum, acc) => sum + Number(acc.current_balance), 0)) || 0;
    ...
    const last30DaysInflow = (transactions == null ? void 0 : transactions.filter((t) => Number(t.amount) > 0).reduce((sum, t) => sum + Number(t.amount), 0)) || 0;
    const last30DaysOutflow = (transactions == null ? void 0 : transactions.filter((t) => Number(t.amount) < 0).reduce((sum, t) => sum + Math.abs(Number(t.amount)), 0)) || 0;
    ...
    const mtdRevenue = (mtdTransactions == null ? void 0 : mtdTransactions.filter((t) => t.transaction_type === "revenue").reduce((sum, t) => sum + Number(t.amount), 0)) || 0;
    const mtdExpenses = (mtdTransactions == null ? void 0 : mtdTransactions.filter((t) => ["expense", "payroll", "tax"].includes(t.transaction_type)).reduce((sum, t) => sum + Math.abs(Number(t.amount)), 0)) || 0;
    ...
    const ytdRevenue = (ytdTransactions == null ? void 0 : ytdTransactions.filter((t) => t.transaction_type === "revenue").reduce((sum, t) => sum + Number(t.amount), 0)) || 0;
    const ytdExpenses = (ytdTransactions == null ? void 0 : ytdTransactions.filter((t) => ["expense", "payroll", "tax"].includes(t.transaction_type)).reduce((sum, t) => sum + Math.abs(Number(t.amount)), 0)) || 0;
    ...
    const totalAR = (arData == null ? void 0 : arData.reduce((sum, ar) => sum + Number(ar.amount_outstanding), 0)) || 0;
    const arAging = {
      current: (arData == null ? void 0 : arData.filter((ar) => ar.aging_bucket === "current").reduce((sum, ar) => sum + Number(ar.amount_outstanding), 0)) || 0,
      "30": (arData == null ? void 0 : arData.filter((ar) => ar.aging_bucket === "30").reduce((sum, ar) => sum + Number(ar.amount_outstanding), 0)) || 0,
      "60": (arData == null ? void 0 : arData.filter((ar) => ar.aging_bucket === "60").reduce((sum, ar) => sum + Number(ar.amount_outstanding), 0)) || 0,
      "90": (arData == null ? void 0 : arData.filter((ar) => ar.aging_bucket === "90").reduce((sum, ar) => sum + Number(ar.amount_outstanding), 0)) || 0,
      "120_plus": (arData == null ? void 0 : arData.filter((ar) => ar.aging_bucket === "120_plus").reduce((sum, ar) => sum + Number(ar.amount_outstanding), 0)) || 0
    };
    const { data: apData } = await supabase.from("accounts_payable").select("amount_outstanding, priority, due_date").eq("status", "pending");
    const totalAP = (apData == null ? void 0 : apData.reduce((sum, ap) => sum + Number(ap.amount_outstanding), 0)) || 0;
    const apByPriority = {
      payroll: (apData == null ? void 0 : apData.filter((ap) => ap.priority === "payroll").reduce((sum, ap) => sum + Number(ap.amount_outstanding), 0)) || 0,
      taxes: (apData == null ? void 0 : apData.filter((ap) => ap.priority === "taxes").reduce((sum, ap) => sum + Number(ap.amount_outstanding), 0)) || 0,
      critical: (apData == null ? void 0 : apData.filter((ap) => ap.priority === "critical").reduce((sum, ap) => sum + Number(ap.amount_outstanding), 0)) || 0,
      normal: (apData == null ? void 0 : apData.filter((ap) => ap.priority === "normal").reduce((sum, ap) => sum + Number(ap.amount_outstanding), 0)) || 0
    };
    ...
    const revenueByServiceLine = await Promise.all(
      (serviceLines || []).map(async (line) => {
        ...
        return {
          name: line.name,
          revenue: (data == null ? void 0 : data.reduce((sum, t) => sum + Number(t.amount), 0)) || 0
        };
      })
    );
    return {
      ...
      bankFeedStatus: ((_a = accounts == null ? void 0 : accounts[0]) == null ? void 0 : _a.sync_status) || "unknown",
      lastBankSync: ((_b = accounts == null ? void 0 : accounts[0]) == null ? void 0 : _b.last_synced_at) || null
    };
  };
  const getAccountingIntegrity = async () => {
    ...
    return {
      bankFeeds: (accounts == null ? void 0 : accounts.map((acc) => ({
...
const _sfc_main$6 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const staleDataSources = computed(() => {
      var _a;
      if (!((_a = integrity.value) == null ? void 0 : _a.dataFreshness)) return [];
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        ...
        if ((_a = unref(integrity).bankFeeds[0]) == null ? void 0 : _a.lastSync) {
...
const useTaxSafety = () => {
  ...
  const getTaxObligations = async () => {
    ...
    const totalExpected = (allReserves == null ? void 0 : allReserves.reduce((sum, r) => sum + Number(r.amount_expected), 0)) || 0;
    const totalReserved = (allReserves == null ? void 0 : allReserves.reduce((sum, r) => sum + Number(r.amount_reserved), 0)) || 0;
...
const useFounderCompensation = () => {
  ...
  const getCompensationSummary = async () => {
    ...
    const now = /* @__PURE__ */ new Date();
    const startOfYear = new Date(now.getFullYear(), 0, 1);
    const { data: events } = await supabase.from("founder_compensation_events").select("event_type, amount, event_date").eq("founder_id", (_a = user.value) == null ? void 0 : _a.id).gte("event_date", startOfYear.toISOString().split("T")[0]).order("event_date", { ascending: false });
    const salaryYTD = (events == null ? void 0 : events.filter((e) => e.event_type === "salary").reduce((sum, e) => sum + Number(e.amount), 0)) || 0;
    const distributionsYTD = (events == null ? void 0 : events.filter((e) => e.event_type === "distribution").reduce((sum, e) => sum + Number(e.amount), 0)) || 0;
    return {
      salaryYTD,
      distributionsYTD,
  ...
  const calculateSafeDistribution = async () => {
    ...
    const payrollReserve = (apData == null ? void 0 : apData.filter((ap) => ap.priority === "payroll").reduce((sum, ap) => sum + Number(ap.amount_outstanding), 0)) || 0;
    const taxReserve = (apData == null ? void 0 : apData.filter((ap) => ap.priority === "taxes").reduce((sum, ap) => sum + Number(ap.amount_outstanding), 0)) || 0;
    const apReserve = (apData == null ? void 0 : apData.filter((ap) => ["critical", "normal"].includes(ap.priority)).reduce((sum, ap) => sum + Number(ap.amount_outstanding), 0)) || 0;
...
const useBillingQueue = () => {
  ...
  const getWorkQueue = async () => {
    ...
    const grouped = queue == null ? void 0 : queue.reduce((acc, item) => {
    ...
    return {
      queue: queue || [],
      grouped: grouped || {},
      count: (queue == null ? void 0 : queue.length) || 0
    };
  };
  ...
  const performAction = async (queueItemId, action, claimId, data) => {
    ...
    await supabase.from("audit_log").insert({
      ...
      actor_id: (_a = user.value) == null ? void 0 : _a.id,
      actor_role: "biller",
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
        ...
        if (unref(queue).count === 0) {
          ...
          ssrRenderList(unref(queue).grouped, (items, action) => {
            ...
            ssrRenderList(items, (item) => {
              ...
              _push(`<div class="queue-item" data-v-4295016d><div class="item-info" data-v-4295016d><span class="claim-number" data-v-4295016d>${ssrInterpolate((_a2 = item.billing_claims) == null ? void 0 : _a2.claim_number)}</span><span class="patient-name" data-v-4295016d>${ssrInterpolate((_b2 = item.billing_claims) == null ? void 0 : _b2.patient_name)}</span><span class="service-date" data-v-4295016d>${ssrInterpolate(formatDate((_c2 = item.billing_claims) == null ? void 0 : _c2.service_date))}</span></div><div class="item-details" data-v-4295016d><span class="payer" data-v-4295016d>${ssrInterpolate((_d2 = item.billing_claims) == null ? void 0 : _d2.payer_name)}</span><span class="amount" data-v-4295016d>${ssrInterpolate(formatCurrency((_e2 = item.billing_claims) == null ? void 0 : _e2.billed_amount))}</span></div><div class="item-reason" data-v-4295016d>${ssrInterpolate(item.blocking_reason)}</div></div>`);
            });
            _push(`<!--]--></div></div>`);
          });
          _push(`<!--]--></div>`);
      ...
      if (unref(selectedItem)) {
        _push(`<div class="modal-overlay" data-v-4295016d><div class="modal" data-v-4295016d><div class="modal-header" data-v-4295016d><h3 data-v-4295016d>Claim Triage: ${ssrInterpolate((_a = unref(selectedItem).billing_claims) == null ? void 0 : _a.claim_number)}</h3><button class="close-button" data-v-4295016d>\xD7</button></div><div class="modal-body" data-v-4295016d><div class="triage-section" data-v-4295016d><h4 data-v-4295016d>What&#39;s Blocking Payment</h4><p data-v-4295016d>${ssrInterpolate(unref(selectedItem).blocking_reason)}</p></div><div class="triage-section" data-v-4295016d><h4 data-v-4295016d>Claim Details</h4><div class="detail-row" data-v-4295016d><span data-v-4295016d>Patient:</span><span data-v-4295016d>${ssrInterpolate((_b = unref(selectedItem).billing_claims) == null ? void 0 : _b.patient_name)}</span></div><div class="detail-row" data-v-4295016d><span data-v-4295016d>Service Date:</span><span data-v-4295016d>${ssrInterpolate(formatDate((_c = unref(selectedItem).billing_claims) == null ? void 0 : _c.service_date))}</span></div><div class="detail-row" data-v-4295016d><span data-v-4295016d>Payer:</span><span data-v-4295016d>${ssrInterpolate((_d = unref(selectedItem).billing_claims) == null ? void 0 : _d.payer_name)}</span></div><div class="detail-row" data-v-4295016d><span data-v-4295016d>Amount:</span><span data-v-4295016d>${ssrInterpolate(formatCurrency((_e = unref(selectedItem).billing_claims) == null ? void 0 : _e.billed_amount))}</span></div><div class="detail-row" data-v-4295016d><span data-v-4295016d>Status:</span><span data-v-4295016d>${ssrInterpolate((_f = unref(selectedItem).billing_claims) == null ? void 0 : _f.claim_status)}</span></div></div><div class="triage-section" data-v-4295016d><h4 data-v-4295016d>Action Required</h4><p data-v-4295016d>${ssrInterpolate(formatActionTitle(unref(selectedItem).action_required))}</p></div></div><div class="modal-footer" data-v-4295016d><button class="action-button primary" data-v-4295016d>${ssrInterpolate(getActionButtonText(unref(selectedItem).action_required))}</button><button class="action-button" data-v-4295016d>Cancel</button></div></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
...
const useTelehealth = () => {
  ...
  const completeTelehealthSession = async (encounterId, sessionEnd, clinicalNotes, resultedInTransport, transportIncidentId) => {
    var _a;
    const sessionStart = new Date(((_a = (await supabase.from("telehealth_encounters").select("session_start").eq("id", encounterId).single()).data) == null ? void 0 : _a.session_start) || "");
  ...
  const getTelehealthEncounters = async (filters) => {
    ...
    if (filters == null ? void 0 : filters.disposition) {
      query = query.eq("disposition", filters.disposition);
    }
    if ((filters == null ? void 0 : filters.isBillable) !== void 0) {
  ...
  const getTelehealthAnalytics = async () => {
    ...
    const totalEncounters = (encounters == null ? void 0 : encounters.length) || 0;
    const billableEncounters = (encounters == null ? void 0 : encounters.filter((e) => e.is_billable).length) || 0;
    const transportDispatched = (encounters == null ? void 0 : encounters.filter((e) => e.resulted_in_transport).length) || 0;
    const totalRevenue = (billingEvents == null ? void 0 : billingEvents.reduce((sum, e) => sum + Number(e.charge_amount), 0)) || 0;
    const paidRevenue = (billingEvents == null ? void 0 : billingEvents.filter((e) => e.billing_status === "paid").reduce((sum, e) => sum + Number(e.charge_amount), 0)) || 0;
    return {
      totalEncounters,
      billableEncounters,
      transportDispatched,
...
const useSystemHealth = () => {
  ...
  const acknowledgeAlert = async (alertId) => {
    ...
    const { error } = await supabase.from("dashboard_alerts").update({
      ...
      acknowledged_by: (_a = user.value) == null ? void 0 : _a.id
    }).eq("id", alertId);
    if (error) throw error;
  };
...
//# sourceMappingURL=dashboard-CS_4XqtU.mjs.map
