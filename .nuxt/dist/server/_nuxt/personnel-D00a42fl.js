  ...
  setup(__props) {
    ...
    const filteredPersonnel = computed(() => {
      ...
      return (personnel2.value || []).filter(
        (p) => p.first_name.toLowerCase().includes(query) || p.last_name.toLowerCase().includes(query) || p.badge_number?.toLowerCase().includes(query)
      );
    });
    const expiringCertifications = computed(() => {
      ...
      const expiring = [](personnel2.value || []).forEach((p) => {
        p.fire_certifications?.forEach((cert) => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(filteredPersonnel.value, (person) => {
        ...
        if (person.specializations?.length) {
        ...
        if (person.fire_certifications?.length > 0) {
...
//# sourceMappingURL=personnel-D00a42fl.js.map
